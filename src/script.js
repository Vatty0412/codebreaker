let attempts = 5;
// injecting attempts into the UI
document.querySelector('#attempts').insertAdjacentText('beforeend', attempts);

// generating a random 6-digit sequence
...
form.addEventListener('input', (e) => {
    ...
    if (val.length) target.nextElementSibling.focus();
});
...
form.addEventListener('submit', (e) => {
    ...
    userResponse.forEach((val, idx) => {
        let clr = '#f87171';
        if (val === sequence[idx]) clr = '#34d399';
        else {
            ansCorrect = false;
            if (sequence.includes(val)) clr = '#fbbf24';
        }
